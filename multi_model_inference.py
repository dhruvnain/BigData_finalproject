...
class MultiHeadCrossAttention(nn.Module):
    ...
    def forward(self, text_features, image_features):
        ...
        Q_text = Q_text.view(
            Q_text.size(0), -1, self.num_heads, self.head_dim
        ).transpose(1, 2)
        K_text = K_text.view(
            K_text.size(0), -1, self.num_heads, self.head_dim
        ).transpose(1, 2)
        V_text = V_text.view(
            V_text.size(0), -1, self.num_heads, self.head_dim
        ).transpose(1, 2)

        Q_image = Q_image.view(
            Q_image.size(0), -1, self.num_heads, self.head_dim
        ).transpose(1, 2)
        K_image = K_image.view(
            K_image.size(0), -1, self.num_heads, self.head_dim
        ).transpose(1, 2)
        V_image = V_image.view(
            V_image.size(0), -1, self.num_heads, self.head_dim
        ).transpose(1, 2)

        # Calculate the attention scores
        attn_scores_text_image = torch.matmul(Q_text, K_image.transpose(-1, -2)) / (
            self.head_dim**0.5
        )
        attn_scores_image_text = torch.matmul(Q_image, K_text.transpose(-1, -2)) / (
            self.head_dim**0.5
        )
        ...
        attn_output_text_image = (
            attn_output_text_image.transpose(1, 2)
            .contiguous()
            .view(text_features.size(0), -1)
        )
        attn_output_image_text = (
            attn_output_image_text.transpose(1, 2)
            .contiguous()
            .view(image_features.size(0), -1)
        )
...
class MultiModalModel(nn.Module):
    def __init__(self, num_labels):
        ...
        self.mhca = MultiHeadCrossAttention(
            text_dim=768, image_dim=1280, num_heads=4, hidden_dim=512, output_dim=2048
        )

        self.classifier = nn.Sequential(
            nn.Linear(2816, 512), nn.ReLU(), nn.Dropout(0.2), nn.Linear(512, num_labels)
        )
    ...
    def forward(self, input_ids, attention_mask, image, device):
        ...
        text_features = outputs["last_hidden_state"][
            :, 0, :
        ]  # CLS token output as text feature
...
def get_inference_data(dataset, minio_client, bucket_name):
    ...
    return DataLoader(inference_dataset, batch_size=BATCH_SIZE, shuffle=False, drop_last = False)
