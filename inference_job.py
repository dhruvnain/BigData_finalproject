...
class InferenceJob:
    def __init__(
        self, bucket: str, final_bucket: str, minio_client: Any, column: str, task: str
    ):
        self.bucket = bucket
    ...
    def get_inference(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        results = classifier(
            inputs, self.labels, hypothesis_template=self.hypothesis_template
        )

        # Set the results to a new column in the dataframe
        labels = [
            result["labels"][0] if result is not None else np.nan for result in results
        ]
        scores = [
            result["scores"][0] if result is not None else np.nan for result in results
        ]
    ...
    def maybe_load_classifier(self, model: Optional[str] = "facebook/bart-large-mnli"):
        if not self.classifier:
            ...
            self.classifier = pipeline(self.task, model=model, device=device)
