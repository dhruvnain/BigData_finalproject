...
class ETLDiskJob(ProcessData):
            ...
            for file in files:
                ...
                if not checked_obj:
                    ...
                    if len(cached) > 0:
                        processed_df = self.extract_information_from_docs(cached)
                        processed.append(processed_df)
                    processed_df = reduce(lambda df1, df2: df1.unionByName(df2), processed)
                    #processed_df = pd.concat(processed).reset_index(drop=True)
                    spark = SparkSession.builder \
                    .master("local[*]") \
                    .appName("ETLPipeline") \
                    .getOrCreate()
                    #df_spark = spark.createDataFrame(processed_df)
                    df_w_image_path = processed_df.filter(processed_df.image.contains("https"))
                    #processed_df = self.spark.read.csv(f'/Users/dhruvnain/Desktop/wildlife_pipeline_on_dataproc/dataframe_folder/{file}', header=True, inferSchema=True)
                    #dataframe = pd.read_csv(f'/Users/dhruvnain/Desktop/wildlife_pipeline_on_dataproc/dataframe_folder/{file}')
                    #dimensions = processed_df.shape
                    #print("Dimensions of DataFrame: (rows, columns)", dimensions)
                    #column_names = spark_df.columns
                    #print("Column names:", column_names)
                    #num_columns = len(spark_df.columns)
                    #print("Number of columns spark_df:", num_columns)
                    #num_rows = spark_df.count()
                    #print("Number of rows spark_df:", num_rows)
                    
                    #processed_df = reduce(lambda df1, df2 : df1.unionByName(df2), processed)
                    if self.minio_client:
                        self.load_file_to_minio(final_filename, processed_df)
                    else:
                        final_filename = final_filename+"spark.csv"
                        print("Filename: ",final_filename)
                    if self.minio_client:
                        image_bucket = f"images-{date}"
                    else:
                        image_bucket = None
                    #df_w_image_path = spark_df.filter(spark_df.image.contains("https"))
                    spark_df = self.perform_classification(df_w_image_path, image_bucket)
                    if not spark_df.rdd.isEmpty():
                        if self.minio_client:
                            self.load_file_to_minio(final_filename, spark_df)
                        else:
                            pandas_df = spark_df.toPandas()
                            pandas_df.to_csv(final_filename, index=False, header = True)
                            #processed_df.coalesce(1).write.option("quote", "\"").csv(final_filename, mode="overwrite", header=True)
    ...
    def create_df(self, ads: list) :
                ...
                if not ProcessData.remove_text(text) and not self.maybe_check_bloom(text):
                    ...
                    dict_df = {
                          "url": ad["url"],
                            ...
                            "text": str(text),
                            "domain": str(domain),
                            "retrieved": str(ETLDiskJob.get_time(ad["fetch_time"])),
                            "name": str(None),
        ...
        if len(final_dict) > 0:
            ...
            df_metas['image'].replace('None', pd.NA, inplace=True)

            # Sort the DataFrame first by 'URL' and then by 'image' column where NaN values are last
            df_sorted = df_metas.sort_values(by=['url', 'image'], na_position='last')
            ...
            df_metas = ProcessData.assert_types(df_filtered)
            ...
            df_spark = self.spark.createDataFrame(df_metas)
        return df_spark
        #return df_metas
